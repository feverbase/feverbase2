...
$(document).ready(function () {
  ...
  $('form').on('mousedown', 'label.check.other > input[type="checkbox"]', function (e) {
    e.preventDefault();
  });
  // typing in text box
  $('form').on('keyup', 'label.check.other > input[type="text"]', function () {
    ...
    if ((elem.val() || "").length === 0) {
      ...
      if (
        ...
        newOther.find('input[type="checkbox"]').attr('value', value + 1).prop('checked', false);
...
function deleteOther(num) {
  ...
  // remove
  $('label.check.other:has(input[type="checkbox"][value="' + num + '"])').remove();
